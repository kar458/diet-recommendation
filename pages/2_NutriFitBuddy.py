...
if not st.session_state.page1['is_first_load']:
    ...
    if body == 0:
        ...
        with engine.connect() as conn:
            ...
            sc_text_result = text(
    f"SELECT * FROM StandardCalories WHERE Stage = '{st.session_state.page1['stage']}' AND Body = '{body}' AND Sex = '{st.session_state.page1['sex']}'"
)
            sc_result = conn.execute(sc_text_result).fetchone()
            standard_calories = StandardCalories(*sc_result)

            # Get low carb diet  
            lc_text_result = text("SELECT * FROM LowCarb WHERE Calories = :calories")
            lc_result = conn.execute(lc_text_result, {'calories': standard_calories.low_carb}).fetchone()
            ...
            lcb1_text_result = text("SELECT * FROM Dish WHERE Id = :id")
            lcb1_result = conn.execute(lcb1_text_result, {'id': low_carb_breakfast_detail.id1}).fetchone()

            lcb2_text_result = text("SELECT * FROM Dish WHERE Id = :id")
            lcb2_result = conn.execute(lcb2_text_result, {'id': low_carb_breakfast_detail.id2}).fetchone()

            lcl1_text_result = text("SELECT * FROM Dish WHERE Id = :id")
            lcl1_result = conn.execute(lcl1_text_result, {'id': low_carb_lunch_detail.id1}).fetchone()

            lcl2_text_result = text("SELECT * FROM Dish WHERE Id = :id")
            lcl2_result = conn.execute(lcl2_text_result, {'id': low_carb_lunch_detail.id2}).fetchone()

            lcd1_text_result = text("SELECT * FROM Dish WHERE Id = :id")
            lcd1_result = conn.execute(lcd1_text_result, {'id': low_carb_dinner_detail.id1}).fetchone()

            lcd2_text_result = text("SELECT * FROM Dish WHERE Id = :id")
            lcd2_result = conn.execute(lcd2_text_result, {'id': low_carb_dinner_detail.id2}).fetchone()
            ...
            lc_text_result = text("SELECT * FROM ModerateCarb WHERE Calories = :calories")
            lc_result = conn.execute(lc_text_result, {'calories': standard_calories.moderate_carb}).fetchone()
            ...
            lcb1_text_result = text("SELECT * FROM Dish WHERE Id = :id")
            lcb1_result = conn.execute(lcb1_text_result, {'id': moderate_carb_breakfast_detail.id1}).fetchone()

            lcb2_text_result = text("SELECT * FROM Dish WHERE Id = :id")
            lcb2_result = conn.execute(lcb2_text_result, {'id': moderate_carb_breakfast_detail.id2}).fetchone()

            lcl1_text_result = text("SELECT * FROM Dish WHERE Id = :id")
            lcl1_result = conn.execute(lcl1_text_result, {'id': moderate_carb_lunch_detail.id1}).fetchone()

            lcl2_text_result = text("SELECT * FROM Dish WHERE Id = :id")
            lcl2_result = conn.execute(lcl2_text_result, {'id': moderate_carb_lunch_detail.id2}).fetchone()

            lcd1_text_result = text("SELECT * FROM Dish WHERE Id = :id")
            lcd1_result = conn.execute(lcd1_text_result, {'id': moderate_carb_dinner_detail.id1}).fetchone()

            lcd2_text_result = text("SELECT * FROM Dish WHERE Id = :id")
            lcd2_result = conn.execute(lcd2_text_result, {'id': moderate_carb_dinner_detail.id2}).fetchone()
            ...
            # Get high carb diet
            hc_text_result = text("SELECT * FROM HighCarb WHERE Calories = :calories")
            hc_result = conn.execute(hc_text_result, {'calories': standard_calories.high_carb}).fetchone()
            ...
            lcb1_text_result = text("SELECT * FROM Dish WHERE Id = :id")
            lcb1_result = conn.execute(lcb1_text_result, {'id': high_carb_breakfast_detail.id1}).fetchone()

            lcb2_text_result = text("SELECT * FROM Dish WHERE Id = :id")
            lcb2_result = conn.execute(lcb2_text_result, {'id': high_carb_breakfast_detail.id2}).fetchone()

            lcl1_text_result = text("SELECT * FROM Dish WHERE Id = :id")
            lcl1_result = conn.execute(lcl1_text_result, {'id': high_carb_lunch_detail.id1}).fetchone()

            lcl2_text_result = text("SELECT * FROM Dish WHERE Id = :id")
            lcl2_result = conn.execute(lcl2_text_result, {'id': high_carb_lunch_detail.id2}).fetchone()

            lcd1_text_result = text("SELECT * FROM Dish WHERE Id = :id")
            lcd1_result = conn.execute(lcd1_text_result, {'id': high_carb_dinner_detail.id1}).fetchone()

            lcd2_text_result = text("SELECT * FROM Dish WHERE Id = :id")
            lcd2_result = conn.execute(lcd2_text_result, {'id': high_carb_dinner_detail.id2}).fetchone()
        ...
        # Cardio
        with engine.connect() as conn:
            cardio_text_result = text("SELECT * FROM Cardio WHERE Stage = :stage and Body = :body and Sex = :sex")
            cardio_result = conn.execute(cardio_text_result, {'stage': st.session_state.page1['stage'], 'body': body, 'sex': st.session_state.page1['sex']}).fetchone()
        ...
        with col1:
            with engine.connect() as conn:
                lower_gym_text_result = text("SELECT * FROM Gym WHERE Day = 'lower'")
                lower_gym_result = conn.execute(lower_gym_text_result).fetchall()

            lower_gym = []
            for lg_result in lower_gym_result:
                lg = Gym(*lg_result)
                with engine.connect() as conn:
                    exercise_text_result = text("SELECT * FROM Exercise WHERE Id = :id")
                    exercise_result = conn.execute(exercise_text_result, {'id': lg.exercise}).fetchone()
        ...
        with col2:
            with engine.connect() as conn:
                upper_gym_text_result = text("SELECT * FROM Gym WHERE Day = 'upper'")
                upper_gym_result = conn.execute(upper_gym_text_result).fetchall()
            ...
            for ug_result in upper_gym_result:
                ...
                with engine.connect() as conn:
                    exercise_text_result = text("SELECT * FROM Exercise WHERE Id = :id")
                    exercise_result = conn.execute(exercise_text_result, {'id': ug.exercise}).fetchone()
                    exercise = Exercise(*exercise_result)
