...
with engine.connect() as conn:
    exercise_statement = text("SELECT*FROM Exercise")
    print(exercise_statement)
    all_exercise_results = conn.execute(exercise_statement).fetchall()
exercise_keywords = ['',]
...
if exercise_keyword != '':
    with engine.connect() as conn:
        exercise_statement = text("SELECT * FROM Exercise WHERE Name = :name")
        exercise_result = conn.execute(exercise_statement, {'name': exercise_keyword}).fetchone()
        if exercise_result:
